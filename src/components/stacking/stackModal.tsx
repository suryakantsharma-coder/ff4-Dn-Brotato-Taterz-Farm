...
function StackModal({ open, onClose }: { open: boolean; onClose: (isSuccess: boolean) => void }) {
  const { address } = useWallet();
  const [txnState, setTxnState] = useState(false);
  const { data, isError, isFetching, handleNfts } = useNFT({
  ...
  }, [isTransactionSuccess, isPending, isTransactionPending, isError, isContractError]);
  ...
  return (
    ...
    >
      {!txnState && (
        <div
          className="w-[80%] lg:w-[28%] 2xl:w-[26%] max-h-[60%] lg:max-h-[60%] 2xl:max-h-[40%] bg-[#2B476E] rounded-[20px] px-2 overflow-auto"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="w-[100%] h-[100%] flex justify-between items-center px-3 sticky top-0 bg-[#2B476E] ">
            <p className="text-5xl">Send to farm</p>
      ...
      {txnState && (
        <div className="w-[70%] lg:w-[25%] 2xl:w-[26%] max-h-[60%] lg:max-h-[60%] 2xl:max-h-[40%]  bg-[#2B476E] rounded-[20px] p-2 overflow-auto">
          <div className="w-[100%] h-[100%] flex flex-cols justify-center items-center text-center ">
            ...
            <br />
            {isPending && <h1 className="text-5xl">Sending To Farm...</h1>}
            <br />
            {isTransactionSuccess && <h1 className="text-5xl">Staked Successfully</h1>}
          </div>
        </div>
      )}
    </div>
